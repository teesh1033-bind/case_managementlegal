...
if ($role === 'admin') {
    ...
    $shortcutsHtml = '
        <button type="button" class="cb-shortcut chat-shortcut" data-prompt="Show my cases">My cases</button>
        <button type="button" class="cb-shortcut chat-shortcut" data-prompt="My upcoming appointments">Appointments</button>
        <button type="button" class="cb-shortcut chat-shortcut" data-prompt="What is my payment balance?">Payments</button>
        <button type="button" class="cb-shortcut chat-shortcut" data-prompt="Documents on my cases">Documents</button>
        <a href="client-cases.php" class="cb-shortcut cb-shortcut--outline">Open cases</a>';
}
...
if ($role === 'client') {
    $mainContentHtml = '
			<div class="cb-hero-card">
				...
				<p class="cb-hero-sub">Ask about your cases, appointments, payments, documents, and court dates. Answers use live data from your account.</p>
				<p class="cb-hero-meta">Logged in as ' . $displayName . '</p>
			</div>
			<div class="cb-layout">
				<div class="cb-panel">
					<div class="cb-panel-hdr">
						<div>
							<h5>Conversation</h5>
							<p>Type a question or pick a quick prompt.</p>
						</div>
					</div>
						...
						<div id="chatWindow" class="chat-window mb-3">
							<div class="d-flex mb-3 chat-message-bot">
								...
								<div class="chat-bubble">
									...
									<div class="chat-bubble-body">Hello ' . $displayName . '! I can answer questions using live data from your account — cases, appointments, documents, payments, and court dates.' . $welcomeHint . '</div>
								</div>
							</div>
						</div>
	...
	<script>
		...
		function appendMessage(sender, html, links) {
			...
			if (sender === 'You') {
				...
				var botIcon = document.body.classList.contains('client-chatbot-page')
					? '<div class="cb-bot-avatar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>'
					: '<div class="icon icon-shape icon-sm me-2 bg-gradient-dark shadow text-center"><i class="ni ni-chat-round text-white opacity-10"></i></div>';
				row.innerHTML = botIcon + '<div class="chat-bubble"><span class="chat-bubble-label">' + escapeHtml(assistantName) + ':</span><div class="chat-bubble-body">' + formatReply(html) + linksHtml + '</div></div>';
			}
		...
		function appendTyping() {
			...
			var thinkIcon = document.body.classList.contains('client-chatbot-page')
				? '<div class="cb-bot-avatar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>'
				: '<div class="icon icon-shape icon-sm me-2 bg-gradient-dark shadow text-center"><i class="ni ni-chat-round text-white opacity-10"></i></div>';
			row.innerHTML = thinkIcon + '<div class="chat-bubble text-muted">Thinking...</div>';
			chatWindow.appendChild(row);
			chatWindow.scrollTop = chatWindow.scrollHeight;
		}
